...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_ent = attention_entropy(attn)

        out = einsum('b i j, b j d -> b i d', attn, v)
        out = rearrange(out, '(b h) n d -> b n (h d)', h = h)
        return self.to_out(out), attn_ent
...
class TNT(nn.Module):
    ...
    def forward(self, x, return_ent=False):
        ...
        for pixel_attn, pixel_ff, pixel_to_patch_residual, patch_attn, patch_ff in self.layers:
            ...
            pixels_, ent1 = pixel_attn(pixels)
            pixels = pixels + pixels_
            ...
            patches_, ent2 = patch_attn(patches)
            patches = patches + patches_
            patches = patch_ff(patches) + patches

            total_ent += (ent1 + ent2) / 2
        cls_token = patches[:, 0]
        if return_ent:
            return self.mlp_head(cls_token), total_ent / len(self.layers)
